...
namespace SweatyBot.Services
{
    public class AudioService : CustomService
    {
        ...
        // Downloader.
        private AudioDownloader m_AudioDownloader = null;
        ...
        public AudioService()
        {
            this.m_AudioDownloader = new AudioDownloader(this); // Only downloaded on playlist add.
        }
        ...
        //Fucks off
        public async Task FuckOffAsync(IGuild guild)
        {
            Log($"Fucking off...", (int)LogType.Text);
            if (m_AudioPlayer.IsRunning()) this.StopAudio();
            await Task.Delay(100);
            await this.LeaveAudioAsync(guild);
        }
        ...
        public void PrintPlaylist()
        {
            ...
            if (count == 0)
            {
                Log("There are currently no items in the playlist.", (int)LogType.Text);
                return;
            }
        ...
        public async Task PlaylistAddAsync(string path)
        {
            ...
            if (audio != null)
            {
                ...
                Log($"Added to playlist : {audio.Title}", (int)LogType.Text);
        ...
        public void PrintLocalSongs(int page)
        {
            ...
            if (itemCount == 0)
            {
                Log("No local files found.", (int)LogType.Text);
                return;
            }
            ...
            {
                Log($"There are {pages} pages. Select page 1 to {pages}.", (int)LogType.Text);
                return;
            }
